                        ...
                        <div>
                            ...
                            <div className='grid grid-cols-1 sm:grid-cols-2 gap-3'>
                                {settings?.paymentModes?.cod && (
                                    <label
                                        className={`flex items-center gap-3 border px-4 py-3 rounded-xl cursor-pointer transition-all duration-200 ${selectedPaymentMethod === "cod"
                                            ? "bg-blue-50 text-blue-600 border-blue-200"
                                            : "bg-gray-50 text-gray-800 border-gray-200 hover:border-blue-200"
                                            }`}
                                    >
                                        <input
                                            type="radio"
                                        ...
                                        <div>
                                            ...
                                            <p className='text-xs text-gray-500'>Pay when you receive order</p>
                                        </div>
                                    </label>
                                )}

                                {settings?.paymentModes?.online?.filter(mode => mode.isActive).length > 0 && (
                                    <label
                                        className={`flex items-center gap-3 border px-4 py-3 rounded-xl cursor-pointer transition-all duration-200 ${selectedPaymentMethod === "online"
                                            ? "bg-blue-50 text-blue-600 border-blue-200"
                                            : "bg-gray-50 text-gray-800 border-gray-200 hover:border-blue-200"
                                            }`}
                                    >
                                        ...
                                        <div>
                                            ...
                                            <p className='text-xs text-gray-500'>Pay first and upload proof</p>
                                        </div>
                                    </label>
                                )}
                            </div>
                            ...
                            {selectedPaymentMethod === "online" && (
                                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4'>
                                    {settings.paymentModes.online.filter(mode => mode.isActive).map(method => {
                                        const isSelected = selectedGateway === method.bankName

                                        return (
                                            <div
                                                key={method.bankName}
                                                className={`relative space-y-1 border p-4 cursor-pointer rounded-xl transition-all duration-200 ${isSelected
                                                    ? "text-blue-600 bg-blue-50 border-blue-300 ring-2 ring-blue-100"
                                                    : "text-gray-800 bg-gray-50 border-gray-200 hover:border-blue-200"
                                                    }`}
                                            ...
                                            >
                                                ...
                                                <p className='text-sm pr-7'>
                                                    <span className='font-bold'>Bank Name:</span> {method.bankName}
                                                </p>
                                                <p className='text-sm'>
                                                    <span className='font-bold'>Account Name:</span> {method.accountName}
                                                </p>
                                                <p className='text-sm'>
                                                    <span className='font-bold'>Account Number:</span> {method.accountNumber}
                                                </p>
                                            </div>
                                        )
                                    })}
