...
export default function PlatformHeader({ settings, isCustomDomain }) {
    ...
    const handleSearch = () => {
        ...
        navigate(isCustomDomain ? `/products?s=${searchText.trim()}` : `/brand/${settings.brandSlug}/products?s=${searchText.trim()}`)
    }
            ...
            <header className='flex justify-between items-center gap-8 px-3 sm:px-8 py-2 sm:py-3 lg:py-3.5 shad'>
                    ...
                    {!user.userID ?
                        <div className='flex gap-2'>
                            <div className='flex items-center gap-2 transition-all duration-200 ease-linear hover:text-[var(--pr)]'>
                                ...
                                <button className='text-sm font-bold' onClick={() => handleAuthNavigation("/auth/login")}>Login</button>
                            </div>/
                            <button className='text-sm font-bold transition-all duration-200 ease-linear hover:text-[var(--pr)]' onClick={() => handleAuthNavigation("/auth/signup")}>Signup</button>
                        </div>
                        :
                        <button className='transition-all duration-200 ease-linear hover:text-[var(--pr)]' onClick={() => navigate("/user/dashboard")}>
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20" fill="none">
                                <path d="M15 9C15 7.34315 13.6569 6 12 6C10.3431 6 9 7.34315 9 9C9 10.6569 10.3431 12 12 12C13.6569 12 15 10.6569 15 9Z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                                <path d="M22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22C17.5228 22 22 17.5228 22 12Z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                                <path d="M17 17C17 14.2386 14.7614 12 12 12C9.23858 12 7 14.2386 7 17" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                ...
                <div className={`fixed top-0 right-0 w-full h-screen bg-white p-4 overscroll-y-auto z-10 transition-all duration-300 ease-linear ${open ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'}`}>
                    ...
                    <div className='flex flex-col gap-6 mt-10'>
                        ...
                        {user.userID ?
                            <>
                                <button className='text-neutral-800 font-bold transition-all duration-200 ease-linear hover:text-[var(--pr)]' onClick={() => { handleAuthNavigation("/auth/login"); setOpen(false) }}>Login</button>
                                <button className='text-neutral-800 font-bold transition-all duration-200 ease-linear hover:text-[var(--pr)]' onClick={() => { handleAuthNavigation("/auth/signup"); setOpen(false) }}>Signup</button>
                            </>
                            :
                            <button className='text-neutral-800 font-bold transition-all duration-200 ease-linear hover:text-[var(--pr)]' onClick={() => { navigate("/user/dashboard"); setOpen(false) }}>Dashboard</button>
                        }
                    </div>
                </div>
            </header >
