...
export default function ProductBoxFashion({ item, idx, settings, isCustomDomain }) {
    ...
    const isFavourite = user?.favourites?.includes(item._id)
    ...
    return (
        <a key={idx} target="_blank"
            href={isCustomDomain ? `/product/${item.slug}` : `/brand/${settings.brandSlug}/product/${item.slug}`}
        ...
        >
            <div className="relative w-full h-42 sm:h-56 md:h-72 overflow-hidden bg-[#f8f8f8]">
                ...
                <img
                    src={`${import.meta.env.VITE_HOST}${item.mainImageURL}`}
                    alt={item.name}
                    loading="lazy"
                ...
                <div className="absolute inset-0 flex flex-row items-end justify-center gap-5 opacity-0 group-hover:opacity-100 transition-opacity duration-500 ease-out bg-black/5">
                    <button className='group/button relative w-full flex justify-center items-center text-xs text-white bg-[#333] font-bold p-3 hover:text-black border border-[#333] hover:border-black translate-y-full group-hover:translate-y-0 disabled:bg-[#333] transition-all duration-200 ease-linear'
                        disabled={adding}
                    ...
                    >
                        {adding ? <ButtonLoader />
                            :
                            <>
                                <span className='z-1'>{hasMultipleVariants || hasOptions ? "Choose Option" : "Add To Cart"}</span>
                ...
                <div role='button' className="group/heart absolute top-3 right-3 p-2 bg-white rounded-full hover:bg-red-100 transition-all duration-200 ease-linear opacity-0 group-hover:opacity-100 translate-x-5 group-hover:translate-x-0"
                    onClick={handleAddToFavourites}
                >
                    {addingToFavourites ? (
                        <ButtonLoader />
                    ) : (
                        <Heart size={16} className={`transition-all duration-200 ease-linear ${isFavourite ? 'text-red-500 fill-red-500' : 'text-[#333] group-hover/heart:text-red-500'}`} />
                    )}
                </div>
