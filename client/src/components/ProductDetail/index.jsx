...
export default function ProductDetails({ settings, product }) {
    ...
    const [customersViewing, setCustomersViewing] = useState(0)

    const hasVariants = product?.variants && product.variants.length > 0
    const hasOptions = product?.options && product.options.length > 0
    const selectedStock = Number(state.stock || 0)
    const isOutOfStock = selectedStock <= 0
    const isMaxQuantity = Number(state.quantity || 1) >= selectedStock

    const getImagesToShow = () => {
        if (selectedVariant?.imageURL) {
            return [`${import.meta.env.VITE_HOST}${selectedVariant.imageURL}`]
        }
        return product?.mainImageURL ? [`${import.meta.env.VITE_HOST}${product.mainImageURL}`] : []
    }
    ...
    useEffect(() => {
        setCustomersViewing(Math.ceil(Math.random() * 100))
    }, [])
    ...
    useEffect(() => {
        ...
        // Reset options/variant when product changes
        ...
        setState({
            ...
            comparedPrice: Number(product.comparedPrice || 0),
    ...
    useEffect(() => {
        ...
        if (!selectedVariant) {
            ...
            setState(prev => ({
                ...
                comparedPrice: Number(product.comparedPrice || 0),
                stock: baseStock,
                quantity: Math.min(Number(prev.quantity || 1), Math.max(baseStock, 1)),
                selectedOptions: []
            }));
        ...
        setState(prev => ({
            ...
            comparedPrice: Number(selectedVariant.comparedPrice || product.comparedPrice || 0),
            stock: variantStock,
            quantity: Math.min(Number(prev.quantity || 1), Math.max(variantStock, 1)),
            selectedOptions
        }));
    ...
    const handleAddToCart = () => {
        if (!isAuthenticated || !user?._id) {
            return window.toastify("Please login to continue!", "info");
        }
        ...
        if (hasVariants && !selectedVariant) {
            ...
            if (missingOptions.length > 0) {
                return window.toastify(`Please select ${missingOptions[0].name}`, "warning");
            }

            return window.toastify("Please select all options", "warning");
        }

        if (Number(state.stock || 0) <= 0) {
            return window.toastify("Product is out of stock", "error");
        }

        if (Number(state.quantity || 1) > Number(state.stock || 0)) {
            return window.toastify(`Only ${state.stock} item(s) available in stock`, "error");
        }
                            ...
                            <div className="flex flex-col lg:flex-row lg:items-end gap-2.5">
                                <div className="flex w-fit flex-col gap-2">
                                    ...
                                    {/* <div className="flex items-center border border-gray-400 rounded-none">
                                        ...
                                        <button className="px-3 text-xl text-gray-600 font-bold transition-all duration-200 ease-linear hover:text-[var(--pr)]"
                                            onClick={() => {
                                                setState(prev => ({
                                                    ...prev,
                                                    quantity: product.stock && prev.quantity < product.stock
                                                        ? prev.quantity + 1
                                                        : prev.quantity
                                                }))
                                            }}
                                    ...
                                    <div className="flex items-center border border-gray-400 rounded-none">
                                        <button
                                            type="button"
                                            className="px-3 text-xl text-gray-600 font-bold transition-all duration-200 ease-linear hover:text-[var(--pr)] disabled:opacity-40 disabled:cursor-not-allowed"
                                            disabled={Number(state.quantity || 1) <= 1}
                                            onClick={() => {
                                                setState(prev => ({
                                                    ...prev,
                                        ...
                                        <input
                                            ...
                                            readOnly
                                            className="w-10 text-center px-0! py-2.5! border-none! rounded-none!"
                                        />
                                        ...
                                        <button
                                            ...
                                            onClick={() => {
                                                setState(prev => {
                                                    ...
                                                    if (quantity >= stock) {
                                                        window.toastify(`Only ${stock} item(s) available in stock`, "info");
                                                        return prev;
                                                    }
