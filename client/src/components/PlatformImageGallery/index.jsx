...
export default function PlatformImageGallery({ settings }) {
    ...
    return (
        <div className='px-3 pt-8 sm:pt-10 md:pt-12'>
            ...
            {loading ?
                ...
                <div className='grid grid-cols-1 md:grid-cols-5 gap-3 mt-8'>
                    {gallery.map((img, i) => (
                        <Image key={img._id} src={`${import.meta.env.VITE_HOST}${img.imageURL}`} alt={`galler-image-${i}`} className='aspect-4/4 object-cover' />
                    ))
                    }
                </div>
            }
