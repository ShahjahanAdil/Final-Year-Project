...
export default function ManagePayments() {
    ...
    const fetchSearchedPayments = (text, page = 1) => {
        setLoading(true)
        axios.get(`${import.meta.env.VITE_HOST}/seller/manage-payments/search?sellerID=${user._id}&searchText=${text}&page=${page}`)
            .then(res => {
                if (res.status === 200) {
                    setPayments(res.data?.searchedPayments || [])
                    setTotalPages(Math.ceil((res.data?.totalSearchedPayments || 0) / 20));
                }
            })
            .catch(err => console.error('Frontend GET error', err.message))
            .finally(() => setLoading(false))
    ...
    const handleUpdatePaymentStatus = async () => {
        setLoading(true);
        try {
            const res = await axios.put(`${import.meta.env.VITE_HOST}/seller/manage-payments/update-status`, {
                paymentID: selectedPayment._id, status: selectedPayment.status
            });
            if (res.status === 202) {
                const updatedPayments = payments.map(payment => payment._id === selectedPayment._id ? { ...payment, status: selectedPayment.status } : payment)
    ...
    const handleDeletePayment = (id) => {
        ...
        setLoading(true);
        axios.delete(`${import.meta.env.VITE_HOST}/seller/manage-payments/delete/${id}`)
            .then(res => {
                if (res.status === 203) {
    ...
    return (
        <>
            <div className='bg-[#fafafa] px-3 sm:px-5 md:px-8 py-2.5 sm:py-4'>
                ...
                <p className='text-[12px] sm:text-[13px] text-gray-900'>Monitor, manage and control all the payments received on your store.</p>
            </div>
            ...
            <div className='seller-container'>
                ...
                <p className='w-fit text-xs bg-yellow-100 text-yellow-500 px-2 py-0.5 mt-5'>Gateway, Transaction ID and Transaction SS columns are for online payment purpose</p>
                    ...
                    <table className="w-full text-xs text-left rtl:text-right text-gray-600">
                        <thead className="text-[10px] text-[var(--secondary)] bg-[var(--secondary)]/5 uppercase border-b border-gray-200">
                            <tr>
                                ...
                                <th scope="col" className="font-bold px-6 py-4 whitespace-nowrap">Gateway</th>
                                <th scope="col" className="font-bold px-6 py-4 whitespace-nowrap">Transaction ID</th>
                                <th scope="col" className="font-bold px-6 py-4 whitespace-nowrap">Transaction SS</th>
                                <th scope="col" className="font-bold px-6 py-4 whitespace-nowrap">Amount ({currency})</th>
                                <th scope="col" className="font-bold px-6 py-4 whitespace-nowrap">Created At</th>
                                <th scope="col" className="font-bold px-6 py-4 whitespace-nowrap">Status</th>
                                    ...
                                    payments.length > 0 ?
                                        payments.map((payment) => (
                                            <tr key={payment._id} className="bg-white border-b border-gray-200">
                                                ...
                                                <td className="px-6 py-4 capitalize">
                                                    <div className={`w-fit px-2 py-0.5 ${payment.status === "pending" ? "bg-yellow-100 text-yellow-500" :
                                                        payment.status === "processing" ? "bg-amber-100 text-amber-500" : payment.status === "paid" ? "bg-green-100 text-green-500" :
                                                            payment.status === "refunded" ? "bg-red-100 text-red-500" : payment.status === "failed" ? "bg-red-100 text-red-500" : ""}`}
                                                    >
                                                        {payment.status}
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <div className='flex justify-end gap-2 items-center'>
                                                        ...
                                                        />
                                                        <RxTrash className='text-[16px] text-red-500 cursor-pointer hover:text-red-300' onClick={() => handleDeletePayment(payment._id)} />
                                                    </div>
                                                </td>
                                            </tr>
                                        ))
                                        :
                                        <tr className='bg-white'>
                                            <td colSpan={7} className='px-6 py-4 text-center text-red-500'>No payment found!</td>
                                        </tr>
                            }
                        </tbody>
                    </table>
                    ...
                    >
                        ...
                        <div className='flex-1 p-6'>
                            ...
                            <select
                                ...
                                onChange={(e) => setSelectedPayment(prev => ({
