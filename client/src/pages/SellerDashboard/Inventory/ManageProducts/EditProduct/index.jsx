...
export default function EditProduct() {
            ...
            .then(res => {
                ...
                if (status === 200) {
                    ...
                    const fetchedVariants = (product.variants || []).map(variant => ({
                        ...
                        comparedPrice: variant.comparedPrice != null ? Number(variant.comparedPrice) : null,
                        price: variant.price != null ? Number(variant.price) : 0,
                        stock: variant.stock != null ? Number(variant.stock) : 0,
                    }));
    ...
    const handleEditProduct = () => {
        ...
        if (Number(comparedPrice || 0) < 0) {
            return window.toastify("Compared price cannot be negative!", "info");
        }

        if (variants.length > 0) {
            const hasInvalidVariant = variants.some(v =>
                Number(v.price || 0) <= 0 || Number(v.stock || 0) < 0
            );

            if (hasInvalidVariant) {
                return window.toastify("Each variant must have valid price and stock!", "info");
            }
        } else {
            if (Number(stock || 0) < 0) {
                return window.toastify("Stock cannot be negative!", "info");
            }
        }
        ...
        const variantsData = variants.map(variant => ({
            ...
            comparedPrice: variant.comparedPrice != null && variant.comparedPrice !== ""
                ? Number(variant.comparedPrice)
                : null,
                            ...
                            <div className='grid grid-cols-2 md:grid-cols-3 gap-4'>
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        className={`block w-full text-sm !p-3 bg-white border border-gray-300 !rounded-none mt-3 ${variants.length > 0 ? "opacity-60 cursor-not-allowed" : ""
                                            }`}
                                        onChange={handleChange}
                                    />

                                    {variants.length > 0 && (
                                        <p className="text-xs text-amber-600 bg-amber-50 px-2 py-1 mt-2">
                                            Stock is calculated automatically from variant stocks.
                                        </p>
                                    )}
                                </div>
                                ...
                                <div className='flex flex-col justify-center items-center absolute inset-0 p-3 text-gray-500 cursor-pointer transition-all duration-200 ease-linear group-hover:text-[var(--secondary)]' onClick={() => mainImageRef.current.click()}>
                                    {
                                        !mainImage && !state.mainImageURL ? (
                                            ...
                                            <div className='h-full'>
                                                <img
                                                    src={mainImagePreview || `${import.meta.env.VITE_HOST}${state.mainImageURL}`}
