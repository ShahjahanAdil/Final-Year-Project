...
router.get("/fetch-main-menu", async (req, res) => {
    try {
        ...
        const mainMenuItems = await menusModel.findOne({ sellerID, name: headerMenuName }).select("links")
...
router.get("/fetch-categories", async (req, res) => {
    try {
        ...
        const categories = await categoriesModel.find({ sellerID, displayOnHome: true })
...
router.get("/fetch-featured-products", async (req, res) => {
    try {
        const { sellerID } = req.query
        const products = await productsModel.find({ sellerID, isFeatured: true, status: 'active' })
...
router.get("/fetch-recent-products", async (req, res) => {
    try {
        const { sellerID } = req.query;
        const products = await productsModel.find({ sellerID, isFeatured: { $ne: true }, status: "active" }).sort({ createdAt: -1 }).limit(12);
...
router.get("/fetch-recent-products-filtered", async (req, res) => {
    try {
        ...
        const products = await productsModel.find(baseFilter).sort({ createdAt: -1 }).limit(12);
...
router.get("/fetch-recent-reviews", async (req, res) => {
    try {
        ...
        const reviews = await reviewsModel.find({ brandSlug }).sort({ createdAt: -1 }).limit(10)
...
router.get("/fetch-footer-menu", async (req, res) => {
    try {
        ...
        const footerMenu1Items = await menusModel.findOne({ sellerID, name: footerMenu1Name }).select("links")
        const footerMenu2Items = await menusModel.findOne({ sellerID, name: footerMenu2Name }).select("links")

        res.status(200).json({ message: 'Main menu items fetched', footerMenu1Items, footerMenu2Items })
    }
    catch (error) {
        console.error(error);
        res.status(500).json({ message: error.message })
